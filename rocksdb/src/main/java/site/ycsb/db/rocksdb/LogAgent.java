  ...
  private static final class WALLogListener extends TailerListenerAdapter {
    ...
    public void handle(String line) {
      ...
      try{
        ...
        // try to get update before, we need to use timestampL - 1.
        // if we use timestampL, we will get update after value
        readOptions.setTimestamp(new Slice(toTimestamp(timestampLong - 1)));
        ...
        // write the log to cdc
        startTs = System.nanoTime();
        writeUpdateToCDCLog(key, updateBefore, updateAfter);
        endTs = System.nanoTime();
        Measurements.getMeasurements().measure("writing_to_cdc",
            (int) ((endTs - startTs) / 1000));

        // now, get the latency for from update to generating cdc
        long writeTimestamp = timestampLong / 1_000_000;
        long latency = System.currentTimeMillis() - writeTimestamp;
